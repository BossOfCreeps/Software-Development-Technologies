"""
•	Функция занесении информации спортсменами о желаемых забегах
•	Функция получения информации судьями о заявленных забегах
•	Функция автоматической генерации распределения по дорожкам и забегам
•	Функция демонстрации информации о распределении спортсменов по дорожкам и забегам
•	Функция занесения судьёй информации о результатах забега спортсмена
•	Функция подсчёта лидеров
•	Функция демонстрации информации о результатах забегов
"""
...
def index(request):
    return render(request, 'index.html', {"devices": None})
    ...
    if str(request.user.groups.all()[0]) == "referee":
        if request.method == "POST":
            if request.POST["but"] == "add":
                Race.objects.filter(sportsmen=request.POST["sportsmen"],
                                    distance=request.POST["distance"]).update(result_time=request.POST["result_time"])
